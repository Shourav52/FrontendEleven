import React, { useContext, useEffect, useState } from 'react'
import { Link, NavLink, useLocation } from 'react-router'
import { AuthContext } from '../Provider/AuthProvider';
import { signOut } from 'firebase/auth';
import auth from '../firebase/firebase.config';
import Donate from '../Pages/Donate/Donate';
import bicon from '../assets/bicon.png'
const Navbar = () => {
  const { user } = useContext(AuthContext);
  const logout = () => {
    signOut(auth)
  }
  const [theme, setTheme] = useState(
    localStorage.getItem("theme") || "light"
  );

  
  useEffect(() => {
    document.documentElement.setAttribute("data-theme", theme);
    localStorage.setItem("theme", theme);
  }, [theme]);

  const handleThemeChange = (e) => {
    setTheme(e.target.checked ? "dark" : "light");
  };

  const location = useLocation();

const involvedRoutes = [
  "/get-blood",
  "/events",
  "/become-a-volunteer",
  "/training",
];

const isGetInvolvedActive = involvedRoutes.includes(location.pathname);

  return (
    <div className='sticky top-0 z-50 bg-base-100'>
      <div className="navbar  bg-gradient-to-r from-red-500 to-red-300 text-yellow-100 shadow-sm">
        <div className="navbar-start">
          <div className="dropdown ">
            <div tabIndex={0} role="button" className="btn btn-ghost lg:hidden">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"> <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6h16M4 12h8m-8 6h16" /> </svg>
            </div>
            <ul
              tabIndex="-1"
              className="menu menu-sm dropdown-content bg-base-100 rounded-box z-1 mt-3 w-52 p-2 shadow">
              <li>
              <NavLink
                to="/allrequest"
                className={({ isActive }) =>
                  isActive
                    ? "bg-red-100 text-red-700 font-semibold"
                    : "text-slate-700"
                }
              >
                All Request
              </NavLink>
            </li>
            <li>
              <NavLink
                to="/search"
                className={({ isActive }) =>
                  isActive
                    ? "bg-red-100 text-red-700 font-semibold"
                    : "text-slate-700"
                }
              >
                Search
              </NavLink>
            </li>
            {user && (
  <li>
    <NavLink
      to="/donate"
      className={({ isActive }) =>
        isActive
          ? "bg-red-100 text-red-700 font-semibold"
          : "text-yellow-200"
      }
    >
      Donate
    </NavLink>
  </li>
)}
            
                        {user  && (
  <li tabIndex={0} className="relative group">
    <button
      className={`px-4 py-2 font-extrabold flex items-center gap-1
        ${isGetInvolvedActive 
          ? "bg-red-100 text-red-700 rounded-lg"
          : "text-yellow-200"}
      `}
    >
      Get Involved
      <svg
        className="w-4 h-4 transform group-hover:rotate-180 transition-transform"
        xmlns="http://www.w3.org/2000/svg"
        fill="none"
        viewBox="0 0 24 24"
        stroke="currentColor"
      >
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
      </svg>
    </button>

    <div className="absolute mt-10 hidden group-hover:grid grid-cols-2 gap-4 bg-white shadow-lg rounded-lg p-4 mt-2 w-80 z-50">
      {/* Column 1 */}
      <div>
        <h4 className="text-sm font-semibold text-gray-500 mb-2">Donate & Request</h4>
        <Link to="/get-blood" className="block text-red-400 px-2 py-1 hover:bg-gray-300 rounded">Donate or Get Blood</Link>
        <Link to="/events" className="block text-red-400 px-2 py-1 hover:bg-gray-300 rounded">Events</Link>
      </div>

      {/* Column 2 */}
      <div className='pt-5'>
        <h4 className="text-sm font-semibold text-gray-500 mb-2">Support</h4>
        <Link to="/become-a-volunteer" className="block text-red-400 px-2 py-1 hover:bg-gray-300 rounded">Become a Volunteer</Link>
        <Link to="/training" className="block text-red-400 px-2 py-1 hover:bg-gray-300 rounded">Request Training</Link>
      </div>
    </div>
  </li>
)}

<li>
              <NavLink
                to="/about"
                className={({ isActive }) =>
                  isActive
                    ? "bg-red-100 text-red-700 font-semibold"
                    : "text-slate-700"
                }
              >About</NavLink>
            </li>
               <li>
              <NavLink
                to="/contact"
                className={({ isActive }) =>
                  isActive
                    ? "bg-red-100 text-red-700 font-semibold"
                    : "text-yellow-200"
                }
              >Contact</NavLink>
            </li>
            <li>
              <NavLink
                to="/blog"
                className={({ isActive }) =>
                  isActive
                    ? "bg-red-100 text-red-700 font-semibold"
                    : "text-yellow-200"
                }
              >Blog</NavLink>
            </li>
            <li>
              <NavLink
                to="/support"
                className={({ isActive }) =>
                  isActive
                    ? "bg-red-100 text-red-700 font-semibold"
                    : "text-yellow-200"
                }
              >Support</NavLink>
            </li>
            <li>
              <NavLink
                to="/privacy-policy"
                className={({ isActive }) =>
                  isActive
                    ? "bg-red-100 text-red-700 font-semibold"
                    : "text-yellow-200"
                }
              >Privacy</NavLink>
            </li>

       </ul>

            
          </div>
         <div className='flex justify-center items-center gap-3'>
          <Link to={'/'}> <img className='w-11 h-10 rounded-4xl' src={bicon} alt="" /></Link>
         
           <Link to={'/'} className="text-xl font-bold ">Blood Donation Center</Link>
         </div>
        </div>
        <div className="navbar-center hidden lg:flex">
          <ul className="menu menu-horizontal px-1 font-extrabold">
            <li>
              <NavLink
                to="/allrequest"
                className={({ isActive }) =>
                  isActive
                    ? "bg-red-100 text-red-700 font-semibold"
                    : "text-yellow-200"
                }
              >
                All Request
              </NavLink>
            </li>
            <li>
              <NavLink
                to="/search"
                className={({ isActive }) =>
                  isActive
                    ? "bg-red-100 text-red-700 font-semibold"
                    : "text-yellow-200"
                }
              >
                Search
              </NavLink>
            </li>
            {user && (
  <li>
    <NavLink
      to="/donate"
      className={({ isActive }) =>
        isActive
          ? "bg-red-100 text-red-700 font-semibold"
          : "text-yellow-200"
      }
    >
      Donate
    </NavLink>
  </li>
)}
            
 {user  && (
  <li tabIndex={0} className="relative group">
    <button
      className={`px-4 py-2 font-extrabold flex items-center gap-1
        ${isGetInvolvedActive 
          ? "bg-red-100 text-red-700 rounded-lg"
          : "text-yellow-200"}
      `}
    >
      Get Involved
      <svg
        className="w-4 h-4 transform group-hover:rotate-180 transition-transform"
        xmlns="http://www.w3.org/2000/svg"
        fill="none"
        viewBox="0 0 24 24"
        stroke="currentColor"
      >
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
      </svg>
    </button>

    <div className="absolute mt-10 hidden group-hover:grid grid-cols-2 gap-4 bg-white shadow-lg rounded-lg p-4 mt-2 w-80 z-50">
      {/* Column 1 */}
      <div>
        <h4 className="text-sm font-semibold text-gray-500 mb-2">Donate & Request</h4>
        <Link to="/get-blood" className="block text-red-400 px-2 py-1 hover:bg-gray-300 rounded">Donate or Get Blood</Link>
        <Link to="/events" className="block text-red-400 px-2 py-1 hover:bg-gray-300 rounded">Events</Link>
      </div>

      {/* Column 2 */}
      <div className='pt-5'>
        <h4 className="text-sm font-semibold text-gray-500 mb-2">Support</h4>
        <Link to="/become-a-volunteer" className="block text-red-400 px-2 py-1 hover:bg-gray-300 rounded">Become a Volunteer</Link>
        <Link to="/training" className="block text-red-400 px-2 py-1 hover:bg-gray-300 rounded">Request Training</Link>
      </div>
    </div>
  </li>
)}
            <li>
              <NavLink
                to="/about"
                className={({ isActive }) =>
                  isActive
                    ? "bg-red-100 text-red-700 font-semibold"
                    : "text-yellow-200"
                }
              >About</NavLink>
            </li>
            <li>
              <NavLink
                to="/contact"
                className={({ isActive }) =>
                  isActive
                    ? "bg-red-100 text-red-700 font-semibold"
                    : "text-yellow-200"
                }
              >Contact</NavLink>
            </li>
            <li>
              <NavLink
                to="/blog"
                className={({ isActive }) =>
                  isActive
                    ? "bg-red-100 text-red-700 font-semibold"
                    : "text-yellow-200"
                }
              >Blog</NavLink>
            </li>
            <li>
              <NavLink
                to="/support"
                className={({ isActive }) =>
                  isActive
                    ? "bg-red-100 text-red-700 font-semibold"
                    : "text-yellow-200"
                }
              >Support</NavLink>
            </li>
            <li>
              <NavLink
                to="/privacy-policy"
                className={({ isActive }) =>
                  isActive
                    ? "bg-red-100 text-red-700 font-semibold"
                    : "text-yellow-200"
                }
              >Privacy</NavLink>
            </li>


          </ul>
        </div>
        <div className="navbar-end gap-3">
          <label className="swap swap-rotate ">
          {/* this hidden checkbox controls the state */}
          <input type="checkbox" checked={theme === "dark"} onChange={handleThemeChange} className="theme-controller"  />

          {/* sun icon */}
          <svg
            className="swap-off h-6 w-6 fill-current"
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 24 24">
            <path
              d="M5.64,17l-.71.71a1,1,0,0,0,0,1.41,1,1,0,0,0,1.41,0l.71-.71A1,1,0,0,0,5.64,17ZM5,12a1,1,0,0,0-1-1H3a1,1,0,0,0,0,2H4A1,1,0,0,0,5,12Zm7-7a1,1,0,0,0,1-1V3a1,1,0,0,0-2,0V4A1,1,0,0,0,12,5ZM5.64,7.05a1,1,0,0,0,.7.29,1,1,0,0,0,.71-.29,1,1,0,0,0,0-1.41l-.71-.71A1,1,0,0,0,4.93,6.34Zm12,.29a1,1,0,0,0,.7-.29l.71-.71a1,1,0,1,0-1.41-1.41L17,5.64a1,1,0,0,0,0,1.41A1,1,0,0,0,17.66,7.34ZM21,11H20a1,1,0,0,0,0,2h1a1,1,0,0,0,0-2Zm-9,8a1,1,0,0,0-1,1v1a1,1,0,0,0,2,0V20A1,1,0,0,0,12,19ZM18.36,17A1,1,0,0,0,17,18.36l.71.71a1,1,0,0,0,1.41,0,1,1,0,0,0,0-1.41ZM12,6.5A5.5,5.5,0,1,0,17.5,12,5.51,5.51,0,0,0,12,6.5Zm0,9A3.5,3.5,0,1,1,15.5,12,3.5,3.5,0,0,1,12,15.5Z" />
          </svg>

          {/* moon icon */}
          <svg
            className="swap-on h-6 w-6 fill-current"
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 24 24">
            <path
              d="M21.64,13a1,1,0,0,0-1.05-.14,8.05,8.05,0,0,1-3.37.73A8.15,8.15,0,0,1,9.08,5.49a8.59,8.59,0,0,1,.25-2A1,1,0,0,0,8,2.36,10.14,10.14,0,1,0,22,14.05,1,1,0,0,0,21.64,13Zm-9.5,6.69A8.14,8.14,0,0,1,7.08,5.22v.27A10.15,10.15,0,0,0,17.22,15.63a9.79,9.79,0,0,0,2.1-.22A8.11,8.11,0,0,1,12.14,19.73Z" />
          </svg>
        </label>
          <Link to={"/dashboard"} className="bg-red-500 hover:bg-red-400 hover:font-extrabold text-yellow-100  px-4 py-2 rounded-lg font-semibold  transition">Dashboard</Link>
          {
            user ? <button onClick={logout} className="hover:bg-red-300 bg-gradient-to-r btn from-red-800 to-red-500 text-slate-200">Logout</button> : <Link to={'/login'} className="btn bg-gradient-to-r from-red-800 to-red-500 hover:bg-red-300 text-yellow-100 rounded-lg">Login</Link>
          }
        </div>
      </div>
    </div>
  )
}

export default Navbar
